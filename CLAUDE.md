...
- ❌ Nunca supor que o APK instalado no telefone foi construído a partir DESTE repositório — **CONFIRA O COMMIT DO BUILD**, que o `eas build:list` imprime: `npx eas build:list --platform android --limit 3`. Num build `preview` o JavaScript vem EMBUTIDO, então o rodapé dele diz a versão daquele commit para sempre, por mais que o repositório ande. (⚠️ Esta linha trazia uma MEDIÇÃO como se fosse regra — "os dois únicos builds da conta são de 07/09/2026, dos commits `80e0d79` e `a8c32e4`, que não existem aqui". Era verdade naquele dia e **deixou de ser no mesmo dia**: em 19/09 nasceram dois builds `preview` deste repositório, de `41673d8` e `e66b0dd`. Medição envelhece; o comando que a refaz, não.)
...
- ❌ Nunca supor que os três ambientes do expo.dev (`development`, `preview`, `production`) concordam entre si nem com o `.env` local — são quatro cópias do mesmo segredo, mantidas à mão, e em 19/09/2026 as quatro divergiam; **compare antes de construir**, porque num build `preview` o valor errado fica embutido no APK e só aparece no aparelho
- ❌ Nunca trocar de projeto Supabase sem refazer OS DOIS cadastros do OAuth — o callback `https://<ref>.supabase.co/auth/v1/callback` no **Google Cloud Console** e o `plataformajairo://auth/google` na lista de Redirect URLs do **projeto novo**; o segundo não tem oráculo (o `/authorize` ecoa igual para válido e inválido), então é olhar no painel. E **o login da web continuar funcionando não prova nada**: a web usa o popup, que não tem redirect URI nenhum
- ❌ Nunca reescrever um endereço num trecho de REGISTRO HISTÓRICO ao atualizar documentação — em 19/09/2026 uma substituição em massa trocou o projeto Supabase também na "Causa raiz nº 1" do `AGENTS.md`, que descreve o que o Console tinha **naquele dia**; atualizar ali não corrige nada e apaga a memória de um defeito que custou dez rodadas
