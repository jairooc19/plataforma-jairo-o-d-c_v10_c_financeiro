...
interface TenantListsProps {
  ativas: TenantSyncData[];
  inativas: TenantSyncData[];
 ...
 *
 * Saiu do `TenantManagerModal` porque aquele arquivo passava de 250 linhas de
 * código fazendo três coisas: a moldura modal, o campo de nova empresa e estas
 * listas. Aqui fica só a parte que desenha o que existe hoje para o usuário.
 *
 * 📜 O HISTÓRICO SÓ APARECE QUANDO HÁ HISTÓRICO. Um título "Histórico de
 * empresas" seguido de nada faz o operador procurar o que não veio; a web toma a
 * mesma decisão (`inactiveTenants.length > 0 &&`).
 *
 * 💬 "DADOS PRESERVADOS" É A FRASE QUE FAZ O BOTÃO REABILITAR TER SENTIDO. Sem
 * ela, uma empresa listada como inativa lê-se como registro morto, e ninguém
 * entende por que existiria um botão para trazê-la de volta. Remover aqui é
 * desativar (`is_active = false`), nunca apagar — ver `useTenantManager.ts`.
 *
