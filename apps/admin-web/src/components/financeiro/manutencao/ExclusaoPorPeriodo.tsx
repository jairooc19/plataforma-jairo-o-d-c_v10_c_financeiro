...
import ListaParaExcluir from "./ListaParaExcluir";
 ...
 *
 * Pedido do dono do projeto em 17/09/2026, ampliado na 2ª rodada do mesmo dia
 * com a listagem e as caixas de marcar.
 *
 * ⚠️ É A OPERAÇÃO MAIS DESTRUTIVA DO MÓDULO, e a tela foi desenhada em volta
 * disso. São QUATRO passos, nunca um:
 *
 *   1. LISTAR    → traz os registros do período; todos nascem marcados
 *   2. MARCAR    → desmarque o que deve ficar (ou use os dois botões)
 *   3. CONFERIR  → o banco diz quantos sairiam de verdade, sem apagar nada
  ...
  return (
    <section className="bg-white rounded-3xl border border-red-200 p-6 shadow-sm">
      ...
      <p className="text-[11px] font-bold uppercase text-slate-500 mb-5">
        LISTE OS LANÇAMENTOS DO PERÍODO, MARQUE OS QUE DEVEM SAIR E CONFIRA ANTES DE EXCLUIR.
        NÃO FUNCIONA EM PERÍODO FECHADO. O QUE SAIR FICA NA LIXEIRA, ABAIXO.
      </p>
      ...
      <button type="button" onClick={x.listar}
        ...
        {x.ocupado ? "TRABALHANDO…" : "LISTAR REGISTROS DO PERÍODO"}
      </button>

      {/* ---------- PASSO 2: a lista com as caixas ---------- */}
      {x.lista !== null && (
        <ListaParaExcluir
          lista={x.lista}
          marcados={x.marcados}
          cortada={x.cortada}
          teto={x.tetoDaListagem}
          aoAlternar={x.alternarUm}
          aoMarcarTodos={x.marcarTodos}
          aoDesmarcarTodos={x.desmarcarTodos}
        />
      )}

      {/* ---------- PASSO 3: a conferência ---------- */}
      {x.lista !== null && x.lista.length > 0 && !x.cortada && (
        <button type="button" onClick={x.conferir}
                disabled={x.ocupado || x.marcados.size === 0}
                className="mt-4 flex items-center gap-2 px-6 py-2.5 rounded-xl bg-slate-800 text-white text-xs font-black uppercase tracking-widest disabled:opacity-40">
      ...
      {x.simulacao && (
        <div className="mt-5 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4">
          ...
          {/* ---------- PASSO 4: digitar o número e excluir ---------- */}
      ...
      {/*
        ...
        Só aparece depois de a pessoa ter listado: antes disso, o aviso seria
        "clique em conferir" para quem ainda nem escolheu as datas.
      */}
      {x.lista !== null && x.veredicto.aviso && (
